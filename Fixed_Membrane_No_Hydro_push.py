...
def assemble_willmore(tri_nbs, V, Nfree, tol=1e-5):
    ...
    for d in range(nd):
        ...
        for (ka, ja, la, ia) in ((k,j,l,i), (j,l,i,k), (l,i,k,j), (i,k,j,l)):
            vk = V[ka]; vj = V[ja]; vl = V[la]; vi = V[ia]
            ...
            if ka < Nfree:
                # diag
                row[ptr] = ka; col[ptr] = ka; val[ptr] = cd - ca; ptr += 1
                # (ka,ja)
                if ja < Nfree:
                    row[ptr] = ka; col[ptr] = ja; val[ptr] = ca - cb; ptr += 1
                else:
                    RHS[base  ] += vj[0]*(ca-cb)
                ...
                # (ka,la)
                if la < Nfree:
                    row[ptr] = ka; col[ptr] = la; val[ptr] = cb - cc; ptr += 1
                else:
                    RHS[base  ] += vl[0]*(cb-cc)
                ...
                # (ka,ia)
                if ia < Nfree:
                    row[ptr] = ka; col[ptr] = ia; val[ptr] = cc - cd; ptr += 1
                else:
                    RHS[base  ] += vi[0]*(cc-cd)
...
def plot_mesh(V, V_fixed, T, fig=None, ax=None,
              free_color='b', fixed_color='c',
              surf_color=(0.1, 0.55, 0.85), surf_alpha=1.0):
    """
    ...
    ax.scatter(V[:,0], V[:,1], V[:,2],
               c=free_color, marker='o', label='Free Vertices')
    ax.scatter(V_fixed[:,0], V_fixed[:,1], V_fixed[:,2],
               c=fixed_color, marker='^', label='Fixed Vertices')
...
if __name__ == "__main__":
    ...
    dt = 20 * (mesh_size**3)  # time step size

    Final_time = 20.0 # final time
    Nsteps = int(Final_time / dt)  # number of time steps
    dX = 0*V.flatten()  # initial guess for the update vector
    dX = dX[:3*Nfree]  # only the free DOFs



    for step in range(Nsteps):
        start = time.time()
        #[K,RHS] = willmore_mat_alt(tri_nbs, V, Nfree)
        row, col, val, RHS = assemble_willmore(tri_nbs, V, Nfree)
        K = coo_matrix((val, (row, col)), shape=(Nfree, Nfree)).tocsr()

        # K is (Nfree×Nfree); Kx will be (3*Nfree×3*Nfree)
        Kx = kron(K, eye(3, format='csr'))
        end = time.time()
        print(f"Matrix assembly took {end - start:.4f} seconds")

        # --- then inside your time‐stepping loop: ---

        # Flatten V into a long vector X in column‐major (MATLAB) order:
        X = V.T.ravel(order='F')      # length = 3*nv

        # Truncate RHS to the free‐DOFs only:
        RHSf = RHS[:3*Nfree].copy()   # length = 3*Nfree

        # --- Implicit update: solve for dX ---
        #    ( I + dt*Kx ) dX = -dt * ( Kx * X_free + RHSf - F_push )
        
        F_push = np.zeros(3*Nfree, dtype=float)  # no external forces
        F_push[2] = -2.0*max(0.0,10.0-dt*step)

        start = time.time()
        A = eye(3*Nfree, format='csr') + dt * Kx
        b = -dt * (Kx.dot(X[:3*Nfree]) + RHSf + F_push)
        end = time.time()
        print(f"Setup took {end - start:.4f} seconds")

        start = time.time()
        #dX = spsolve(A, b)
        dX, info = gmres(A, b, tol=1e-4, x0=dX, restart=100, maxiter=1000)
        end = time.time()
        print(f"Solving took {end - start:.4f} seconds")

        # Compute new X
        Xnew = X.copy()
        Xnew[:3*Nfree] = X[:3*Nfree] + dX

        # Reshape back into V:
        V = Xnew.reshape((3, -1), order='F').T
        if step % 100 == 0:
            print(f"Step {step+1}/{Nsteps}, max displacement: {np.max(np.abs(dX))}")
            fig, ax = plot_mesh(V, V[Nfree::,:], T, fig, ax)
